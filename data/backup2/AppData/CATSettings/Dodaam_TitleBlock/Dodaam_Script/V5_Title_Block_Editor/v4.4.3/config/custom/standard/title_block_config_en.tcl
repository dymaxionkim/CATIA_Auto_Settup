...
#
set global_fields(1)   	{Number		    	"Drawing Number" 	entry}
set global_fields(2)   	{Materialnummer    	"Material Number"   entry}
set global_fields(3)   	{AuftragsNummer   	"Order Number"      entry}
set global_fields(4)   	{Projekt  			Project	      		entry}
set global_fields(5)   	{Baugruppe 			Assemblyn	      	entry}
set global_fields(6)   	{Ursprung   		"Derived From" 		entry}
set global_fields(7)   	{ErsatzFuer   		"Replacement for"  	entry}
set global_fields(8)   	{ErsatzDurch   		"Replaced By"     	entry}
set global_fields(9)    {Verwendung			"Usage"				entry}
...
set fields(1)   		{Description		Description				text}
set fields(2)   		{Bearbeitet       	"Designed By"     		entry}
set fields(3)   		{BearbeitetDate    	"Design Date"     		date}
set fields(4)   		{Geprueft         	"Checked By"			entry}
set fields(5)   		{GeprueftDate		"Checked Date"     		date}
set fields(6)   		{Norm         		"Norm Checked"			entry}
set fields(7)  		 	{NormDate    		"Norm Checked Date" 	date}
set fields(8)   		{Freigegeben    	Released	       		entry}
set fields(9)   		{FreigegebenDate	"Release Date"		    date}
#set fields(10)			{SheetScale		    Masstab					entry}
set fields(10)			{Massstab		    Scale					list}
set fields(11)			{Weight		     	Weight					entry}
